...
class ImageNetSubset(BaseRealDataset):
    ...
    def __init__(
        ...
        self.indices = torch.nonzero(mask, as_tuple=False).reshape(-1).tolist()
        self.ds = torch.utils.data.Subset(self.full_ds, self.indices)
        self.targets = [self.convert_label(int(self.full_ds.targets[i])) for i in self.indices]
