...
def main(cfg: DistillCfg):
    ...
    cfg.run_name = wandb.run.name or cfg.run_name or wandb.run.id
    cfg.lr = float(cfg.lr)
    ...
    wandb.finish()
    print("WandB Finshed!")
    if hasattr(distillation, "train_loader"):
        del distillation.train_loader
    if hasattr(distillation, "test_loader"):
        del distillation.test_loader
