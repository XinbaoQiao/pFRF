...
class LinearGM:
    ...
    def __init__(self, cfg: DistillCfg):
        ...
        train_dl_kwargs = dict(
            ...
            pin_memory=True,
            drop_last=True,
            persistent_workers=True if (cfg.num_workers > 0 and self.train_dataset.num_classes <= 50) else False,
        )
        if cfg.num_workers > 0:
            train_dl_kwargs["prefetch_factor"] = 2 if self.train_dataset.num_classes <= 50 else 1
        self.train_loader = DataLoader(self.train_dataset, **train_dl_kwargs)
