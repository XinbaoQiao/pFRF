...
# FL stage order: prioritize short/light datasets first.
FL_DATASETS=(
  "cifar10" "cifar100" "food101" "spawrious" "stanforddogs" "waterbirds"
  "artbench" "cub2011" "imagenet-100" "imagenet-1k"
)
...
METHODS=("fedavg" "fedprox" "ccvr" "fedpcl" "fedncm" "afl" "full_dataset" "lgm")
FL_BASELINES=("fedavg" "fedprox" "ccvr" "fedpcl" "fedncm" "afl")
...
FL_LOCAL_BATCH_SIZE="${FL_LOCAL_BATCH_SIZE:-${LOCAL_BATCH_SIZE:-64}}"
...
PATIENCE_ROUNDS="${PATIENCE_ROUNDS:-20}"
...
run_one_task() {
  ...
  "${PYTHON_BIN}" "${PROJECT_ROOT}/src/baselines/run_baselines.py" \
    ...
    --ccvr_calib_lr "${CCVR_CALIB_LR:-0.05}" \
    "${extra_args[@]}" \
    >> "${log_file}" 2>&1
}
