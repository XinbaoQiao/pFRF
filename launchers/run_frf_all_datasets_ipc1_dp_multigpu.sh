...
MODEL_GPU_QUEUES=(
  ...
  "7|mocov3_resnet50,clip_vitb,dinov2_vitb,eva02_vitb"
)

if [[ -n "${DATASETS_OVERRIDE:-}" ]]; then
  IFS=',' read -r -a DATASETS <<< "${DATASETS_OVERRIDE}"
fi
if [[ -n "${MODEL_GPU_QUEUES_OVERRIDE:-}" ]]; then
  IFS=',' read -r -a MODEL_GPU_QUEUES <<< "${MODEL_GPU_QUEUES_OVERRIDE}"
fi
...
run_model_queue() {
  ...
  for dataset in "${DATASETS[@]}"; do
    ...
    mkdir -p "${ds_root}"
    echo "[$(date '+%F %T')] START gpu=${gpu_id} dataset=${dataset} model=${model} ipc=1 dp_enable=${DP_ENABLE} dp_epsilon=${DP_EPSILON} dp_delta=${DP_DELTA}" | tee -a "${run_log}"
