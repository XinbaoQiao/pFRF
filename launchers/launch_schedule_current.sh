...
LAUNCH_ROOT="${OUTPUT_BASE}/schedule_launch_${RUN_TS}"
mkdir -p "${LAUNCH_ROOT}"
...
# Keep the DP task on its script's intended dataset subset, but preserve the
# same global ordering as COMMON_DATASETS so warm-up remains useful.
DP_DATASETS=(
  food101
  imagenette
  spawrious
...
TASK3_LOG="${LAUNCH_ROOT}/task3_frf56.tmux.log"
TASK4_LOG="${LAUNCH_ROOT}/task4_dp7.tmux.log"

TASK1_SCRIPT="${LAUNCH_ROOT}/task1_warmup.sh"
TASK2_SCRIPT="${LAUNCH_ROOT}/task2_baselines.sh"
TASK3_SCRIPT="${LAUNCH_ROOT}/task3_frf56.sh"
TASK4_SCRIPT="${LAUNCH_ROOT}/task4_dp7.sh"
...
start_tmux_session "schedule_warmup" "${TASK1_SCRIPT}"
start_tmux_session "schedule_baselines" "${TASK2_SCRIPT}"
start_tmux_session "schedule_frf56" "${TASK3_SCRIPT}"
start_tmux_session "schedule_dp7" "${TASK4_SCRIPT}"

printf '%s\n' "${LAUNCH_ROOT}" > "${PROJECT_ROOT}/latest_schedule_launch_root.txt"

echo "Launch root: ${LAUNCH_ROOT}"
echo "Task1 session: schedule_warmup"
echo "Task2 session: schedule_baselines"
echo "Task3 session: schedule_frf56"
echo "Task4 session: schedule_dp7"
